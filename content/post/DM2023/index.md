...
- 邻域$N_G(v)$：与$v$相邻的**点**的集合，$N_G(v)=\left \{u|u\in V(G) \land (u,v)\in E(G) \land u \ne v \right \} $

> 从定义来看，有自环的点的邻域不算自身
>
- 关联集$I_G(v)$：与$v$关联的**边**的集合，$I_G(v)=\left \{e|e与v关联\right \} $
...
- 割点：$v是割点 \Leftrightarrow \left \{v\right \} 是点割集$
- 桥：$(u,v)是桥 \Leftrightarrow \left \{(u,v)\right \} 是边割集$
...
对多面体建模，设图共有$n$个面，各个面编号为$1,2,...,n$。令图$G=<V,W>$，其中点集$V=\left \{v_i|i是一个面\right \} $，边集$E=\left \{(v_i,v_j)|i与j有公共棱\right \} $。则多面体编号为$i$的面的棱的数量就是点$v_i$的度数$d(v_i)$。如果空间中存在有奇数个面且每个面均有奇数条棱的多面体，则$G$的总度数$\sum_{i=1}^nd(v_i)$为奇数。由握手定理，总度数必为偶数，则空间中不可能存在有奇数个面且每个面均有奇数条棱的多面体。
